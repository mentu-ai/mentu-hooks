...
# ///
"""
Genesis Key Reader — workspace constitution for the policy harness (M2b).

VERBATIM port of ``hooks/genesis_reader.py`` (the sanctum copy — source of
truth, newer than the published one). The governance verdicts must stay
byte-identical to the legacy reader: ``tests/test_genesis_port.py`` imports
BOTH this module and ``hooks/genesis_reader.py`` and asserts identical
``resolve_role`` / ``actor_allowed`` across the federator matrix, the
ungoverned default, and corrupt JSON. Carry-overs preserved exactly:

  * federator hard-deny — an ``agent:*`` actor resolving to the ``federator``
    role can never hold it (``actor_allowed`` -> False).
  * the ``contribute`` op in the permission/denied-ops vocabulary.
  * permissive-when-ungoverned — ``actor_allowed`` returns True and
    ``get_allowed_ops`` returns ``["*"]`` when no genesis.json governs.
  * wildcard actor matching (``agent:*`` / ``human:*`` / ``*``).
  * corrupt / non-object JSON -> warn, fall back to ungoverned.
  * ``scope`` is inert — always ``["*"]`` (the legacy hardcode; the Stop gate
    tightens scope only via an explicit ctx override, never via this).

Reads ``.mentu/genesis.json`` (machine-readable twin of genesis.key).
Provides role-based permission checks, tier classification, and scope
constraints. Falls back to permissive defaults when no genesis.json exists.
"""
...
class GenesisReader:
    ...
    def actor_allowed(self, actor: str, operation: str) -> bool:
        ...
        # Hard deny: agent:* actors can never hold the federator role.
        if role == "federator" and actor.startswith("agent:"):
            return False
